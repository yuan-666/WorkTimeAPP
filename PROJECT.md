...
- 最新功能发布提交：`v0.3.2` 手机日历重构与暗色模式修复（待推送/发布）
...
- `v0.3.2` 本轮重点是手机日历布局重构（四宫格+月份独立行+周压缩）、暗色模式全面修复（主题变量替换全部硬编码边框色）、以及多处文案精简。三 agent 并行审查修复了 7 个 UI 问题。
