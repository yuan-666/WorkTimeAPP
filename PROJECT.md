...
- 最新功能发布提交：`v0.3.5` iOS 原生风格重构 + 拇指区交互 + 暗色修复 + 密码修改 + API 安全
- 最新发布说明：`RELEASE_NOTES.md`（推荐 Git 标签与 GitHub Release：`v0.3.5`）
- 版本规则：补丁号只走 `0` 到 `9`；超过 `9` 时进入下一小版本，因此原 `v0.2.10` 记为 `v0.3.0`，当前版本为 `v0.3.5`。
...
- 本轮进行了一次 UX/UI 细节打磨：解决了暗色模式下 toast 提示 `.notice` 在桌面端飘高的 Bug，将 `DESIGN.md` 文档与实际的 iOS Blue 风格对齐，去除了 `.is-today` 僵硬的边框并柔化了表单输入框的 `box-shadow` focus ring，整体保留 `v0.3.5` 原生 HIG 风格并进一步提升优雅度。
- `v0.3.5` 本轮重点是 iOS 原生 HIG 风格全面重构（纯色卡片 + 微交互 + 弹簧缓动）、月份切换和左右滑动翻页、暗色模式完美修复、选中日期空白修复。新增密码修改和 API 限流。
