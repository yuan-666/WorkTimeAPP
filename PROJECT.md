...
- 最新功能发布提交：`v0.2.7` 发布提交
- 最新发布说明：`RELEASE_NOTES.md`（Git 标签与 GitHub Release：`v0.2.7`）
...
- 侧边栏和移动端底部新增动态更新日志入口、`© yuan 版权所有`、GitHub 仓库图标和 yuan6.cn 博客友情链接图标；仍保留 `changelog.html` 独立页面。
...
- `v0.2.7` 修复发布后旧缓存继续显示 `v0.2.5` 的问题：入口资源、模块导入和 Service Worker 注册都加入版本参数。
