...
- 移动端（≤720px）日历改为按天列表视图，每行显示日期、星期、日期类型标记、工时和工资，点击选中日期并打开底部登记抽屉。
...
- `v0.2.8`（待发布）修复侧边栏版本文字溢出、批量补全工时无反应、登录缺少具体错误提示、工资计算需手动添加每天8小时、手机日历需横向滚动五个问题。
- 新增 `autoFillWorkday` 默认设置，会改变现有用户的月度工资显示（无记录工作日自动计入），用户可在设置中关闭。
- 手机端日历从网格改为列表视图，不再显示周合计行。
- 测试从 36 项增加到 41 项，覆盖 `getAutoFilledEntries` 和 `autoFillWorkday` 开关。
