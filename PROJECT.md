...
- 最新功能发布提交：`v0.2.10` 手机端交互与登录修复（待推送/发布）
- 最新发布说明：`RELEASE_NOTES.md`（推荐 Git 标签与 GitHub Release：`v0.2.10`）
...
- 移动端设置页保留四个底部主导航入口，设置详情页使用页内返回按钮；云备份按钮会读取当前二级页表单，手机端可正常登录/同步。
...
- `v0.2.10` 本轮修复重点是手机端交互稳定性、手机端云备份登录、批量处理草稿保留和管理员登录配置容错。
- 新增 `tests/functions.test.js`，测试数从 41 项增加到 43 项，覆盖 ESA 单入口的 RSA 公钥、管理员加密登录和坏密文安全错误。
