...
- `src/calculations.js`：工时、薪资、个税、节假日、休息周期、倒班周期等核心计算。
...
- 可选倒班日历独立于工时登记：未开启时只在设置“功能开关”保留总开关，隐藏倒班导航、倒班页面和倒班详细设置；开启后主导航出现“倒班”。
- 倒班日历默认四段周期为白班、上夜班、下夜班、休班；每段班次名称可按用户班组叫法修改，并按锚点日期与周期开始时间自动推算当前班次、本轮进度、班次结束倒计时和下个休班。
...
- 倒班规则可从工时班次模板复用上下班时间，减少在工时设置和倒班设置之间重复填写。
...
- `v0.3.5` 彻底解决了线上环境 (ESA) 云同步 500 报错（KV 额度超限问题）。
- `v0.3.4` 进行了一次 UX/UI 细节打磨：解决了暗色模式下 toast 提示 `.notice` 在桌面端飘高的 Bug，将 `DESIGN.md` 文档与实际的 iOS Blue 风格对齐，去除了 `.is-today` 僵硬的边框并柔化了表单输入框的 `box-shadow` focus ring。同时优化了移动端设置页的保存按钮布局。
