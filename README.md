...
- 支持阿里云 ESA + KV 云备份：用户界面只显示账号、密码、备份和恢复，不暴露接口配置
- 用户云备份密码会先用浏览器端公钥加密，服务端解密后只保存 PBKDF2-SHA256 加盐哈希
- 管理员后台独立为 `admin.html`，需要管理员账号密码登录后才能查看用户总数、IP 网段分布、同步/登录使用情况，并可改密、停用/启用用户、保存备注
- 支持跟随系统、浅色、深色三种主题模式，深色模式下表单、日历和后台保持一致对比度
- 侧边栏底部提供动态更新日志入口、yuan 版权所有、GitHub 和博客友情链接图标
...
8. 在 `设置` 的数据管理中可使用云同步；如果没有部署后端，仍可定期使用 `备份` 导出本地 JSON。
...
- `/api/cloud`：用户注册、登录、恢复、备份。用户数据写入 `worktimeapp` KV 的 `user:{userId}`；密码由浏览器使用公钥加密后提交，服务端用私钥解密并保存 PBKDF2-SHA256 加盐哈希。
...
- `admin_name`：管理员账号。
- `admin_passwd`：管理员密码；可以是字符串，也可以是 `{ "passwordSalt": "...", "passwordHash": "..." }` 形式的 PBKDF2 记录。
