...
- 支持阿里云 ESA + KV 云备份：用户界面只显示登录和同步操作，不暴露接口配置；手机设置二级页也可直接登录和同步
- 手机版月历页四宫格（税前/工时↔加班/目标/放假↔上班天数），点击切换数据维度，日历可压缩到本周
- 支持深色模式，边框和提示框全部使用主题变量，浅色/深色/系统三档切换
- 用户云备份密码会先用浏览器端公钥加密，服务端解密后只保存 PBKDF2-SHA256 加盐哈希；登录后保存 7 天签名会话，不在本机保存密码
- 管理员后台独立为 `admin.html`，需要管理员账号密码登录后才能查看用户总数、IP 网段分布、同步/登录使用情况，并可改密、停用/启用用户、保存备注
- 支持跟随系统、浅色、深色三种主题模式，深色模式下表单、日历和后台保持一致对比度
- 桌面侧边栏标题右侧提供版本号胶囊，点击可进入更新日志；侧边栏底部展示当前云备份登录状态，点击可进入云备份设置
